...
def nouns_activities(nouns_file, activities_file):
    ...
    id = np.random.randint(10000,100000)  # try to match prompt with asynchronized outputs from server via id
    return f"{IE.a(random.choice(nouns))} {random.choice(activities)}", id
